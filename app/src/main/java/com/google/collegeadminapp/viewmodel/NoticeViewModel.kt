...
import com.google.collegeadminapp.utils.Constant.NOTICE
...
val map=mutableMapOf<String,String>()
        ...
        map["title"]=title
        map["link"]=link
        noticeRef.document(docId).set(map).addOnSuccessListener { _isPosted.postValue(true) }.addOnFailureListener {_isPosted.postValue(false)  }
    }
    fun getNotice(){
    ...
    fun deleteNotice(noticeModel: NoticeModel){
        ...
        noticeRef.document(noticeModel.docId!!).delete().addOnSuccessListener {
            noticeModel.imageUrl?.let { it1 -> Firebase.storage.getReferenceFromUrl(it1).delete() }
